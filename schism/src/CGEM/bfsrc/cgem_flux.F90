...
use cgem, only:ff,ff_flux,Which_fluxes,iO2surf,iO2
use MOD_UTILITIES
...
!---------------------------------------------------------------
 ...
 if(OsDOp.gt.1.25) Vtrans = Vtrans * (OsDOp*OsDOp)
   ...
   O2_atF         = Vtrans*(O2_sfc - alpha_O2*Op)
                                           ...
                                           ! negative means
                                           ! into
   ff_flux(iO2) = - O2_atF/dz(1)
   ff_check = ff(1,iO2) - ff_flux(iO2)*dT
   if(ff_check.lt.0) ff_flux(iO2) = 0.
!   write(6,*) "flux,O2_atF,dz",ff_flux(iO2),O2_atF,dz(1)
